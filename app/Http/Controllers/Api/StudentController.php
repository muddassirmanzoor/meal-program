...
class StudentController extends Controller
{
    ...
    public function submitEnrollment(Request $request)
    {
        ...
        // Check if a meal consumption entry exists for today
        $mealConsumptionExists = DB::table('meal_consumption')
            ->where('emis_code', $emis_code)
            ->whereDate('consumption_date', '=', date('Y-m-d'))
            ->exists();

        if ($mealConsumptionExists) {
            // Prevent update or insert if meal consumption is already entered for today
            return response()->json([
                'status' => 'success',
                'message' => 'Cannot update or create new enrollment after meal consumption entry for today.',
            ], 200);
        }
        ...
        foreach ($classes as $class) {
            ...
            // Check if a record exists for today based on 'updated_at'
            $existingRecord = DB::table('new_enrollments_2024')
                ->where('emis_code', $emis_code)
            ...
            if ($existingRecord) {
                // Update the existing record if it was updated today
                DB::table('new_enrollments_2024')
                    ->where('emis_code', $emis_code)
                    ->where('class', $className)
                    ->update([
                        'boys' => $boys,
                ...
                // Create a new record if no record was updated today
                DB::table('new_enrollments_2024')->insert([
                    'emis_code' => $emis_code,
                    'class' => $className,
                    'boys' => $boys,
                    'girls' => $girls,
    ...
    public function submitEnrollmentNew(Request $request)
    {
        // Validate the incoming request
        $validator = Validator::make($request->all(), [
            'classes' => 'required|array',
...
//        }
        ...
        foreach ($classes as $class) {
            ...
            $existingRecord = DB::table('new_enrollments_2024')
                ->where('emis_code', $emis_code)
                ->where('class', $className)
                ->whereDate('updated_at', Carbon::today()->toDateString()) // Only check the date part
                ->first();
            ...
            if ($existingRecord) {
                ...
                DB::table('new_enrollments_2024')
                    ->where('emis_code', $emis_code)
                    ->where('class', $className)
                    ->whereDate('updated_at', Carbon::today()->toDateString()) // Ensure it checks only the date
                    ->update([
                        'boys' => $boys,
                        'girls' => $girls,
                        'total' => $total,
